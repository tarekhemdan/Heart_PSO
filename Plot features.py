...
for col in adni_df.columns[:-1]: # Exclude the output variable
    fig, ax = plt.subplots()
    sns.histplot(data=adni_df, x=col, hue='output', kde=True, ax=ax)
...
for col in adni_df.columns[:-1]: # Exclude the output variable
    fig, ax = plt.subplots()
    sns.boxplot(data=adni_df, x='output', y=col, ax=ax)
...
sns.pairplot(adni_df, hue='output')
...
for col in adni_df.columns[:-1]: # Exclude the output variable
    fig, ax = plt.subplots()
    sns.violinplot(data=adni_df, x='output', y=col, ax=ax)
    ax.set_title(f"Violin plot of {col}")
    plt.savefig(f"{col}_violinplot.png", dpi=300, bbox_inches='tight')
    plt.show()
    
# Plot scatterplots of each feature against the output variable
for col in adni_df.columns[:-1]: # Exclude the output variable
    fig, ax = plt.subplots()
    sns.scatterplot(data=adni_df, x=col, y='output', ax=ax)
    ax.set_title(f"Scatter plot of {col} vs. output")
